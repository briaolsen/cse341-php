  ...
  try {
    ...
    if ($results && count($results) > 0) {
      ...
      if ($book_result && count($book_result) > 0) {
        $_SESSION['result'] = "This book is not in the library database.";
      } else {
        $book_query = 'DELETE book (title, lexile, genre, author_id) VALUES (?, ?, ?, ?);';
        $book_stmt = $db->prepare($book_query);
        $book_stmt->execute($params);

        $_SESSION['result'] = "You have deleted " . $title . " by " . $authorFirst . " " . $authorLast . " from the library.";
      }
